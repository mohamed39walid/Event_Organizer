...
} elseif ($tickets >= 35 && $tickets < 140) { return 'text-orange-600 dark:text-orange-400' ; } elseif ($tickets==0) {
    return 'text-red-600 dark:text-red-400' ; } return 'text-gray-600 dark:text-gray-400' ; } @endphp
            ...
            <div class="p-6">
                <div
                    ...
                    <div class="flex items-center gap-4">
                        <span class="flex items-center px-3 py-1 text-xs font-medium font-poppins rounded-full
                ...
                @if ($proposals && auth()->user()->role == 'organizer' && $event->user_id == auth()->id())
                <div class="mt-6 border-t border-border dark:border-dark-border pt-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-surface dark:bg-dark-bg rounded-lg p-4 shadow-sm">
                            <div class="flex relative flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                {{-- Status Badge --}}
                                ...
                                <div>
                                    {{-- Title --}}
                                    <h4 class="text-base font-semibold font-inter text-primary dark:text-dark-primary">
                                        {{ $proposal->title }}
                                    </h4>
                                ...
                                <div class="flex space-x-2 mt-5 mr-2">
                                    ...
                                    <button type="button" onclick="openApproveModal({{ $proposal->id }})"
                                        class="px-3 py-1 text-xs cursor-pointer bg-emerald-500 hover:bg-emerald-600 text-white rounded-full">
                                        <i class="fa-solid fa-check"></i>
                                    </button>

                                    {{-- Reject Button --}}
                                    <form action="{{ route('organizer.events.proposals.reject', $proposal->id) }}"
                                        method="POST">
                                        @csrf @method('PUT')
                                ...
                                <div id="approveModal-{{ $proposal->id }}"
                                    ...
                                    <div
                                        ...
                                        <form action="{{ route('organizer.events.proposals.approve', $proposal->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input type="time" id="start_time_{{ $proposal->id }}" name="start_time"
                                                    step="60" value="{{ old('start_time') }}"
                                                    class="w-full px-4 py-2 border border-border dark:border-dark-border rounded-lg bg-white dark:bg-dark-bg text-primary dark:text-dark-primary focus:outline-none focus:ring-2 focus:ring-accent">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <input type="time" id="end_time_{{ $proposal->id }}" name="end_time"
                                                    step="60" value="{{ old('end_time') }}"
                                                    class="w-full px-4 py-2 border border-border dark:border-dark-border rounded-lg bg-white dark:bg-dark-bg text-primary dark:text-dark-primary focus:outline-none focus:ring-2 focus:ring-accent">
                                            ...
                                            <div class="flex justify-end space-x-2">
                                                <button type="button" onclick="closeApproveModal({{ $proposal->id }})"
        ...
        <div class="mt-6 bg-white dark:bg-dark-surface rounded-xl shadow-lg">
            <div class="p-6">
                <h3 class="text-lg font-semibold font-heading text-secondary dark:text-dark-secondary mb-4">
                    ...
                    <button id="openGoogleMapsBtn" class="cursor-pointer dark:text-dark-primary light:text-blue-500 btn btn-primary mt-2">
                        Open in Google Maps
                    </button>
            </div>
        </div>

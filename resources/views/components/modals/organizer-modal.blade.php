...
@if (Auth::user() && Auth::user()->role == 'organizer')
    ...
    {{ session('errors') && !str_starts_with(array_keys(session('errors')->getBags())[0] ?? '', 'approve_') ? 'flex' : 'hidden' }}">
        <div
            class="bg-white dark:bg-dark-bg w-full max-w-2xl rounded-2xl p-8 shadow-2xl relative mx-4 max-h-[90vh] overflow-y-auto">
            <!-- Close Button -->
            ...
            <div class="space-y-4">
                ...
                <form action="{{ route('organizer.events.update-event', ['id' => $event->id]) }}" method="POST"
                    ...
                    <div>
                        ...
                        <select name="status"
                            class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-dark-bg text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-accent focus:outline-none">
                            <option class="text-gray-900 dark:text-gray-100" value="Available"
                                {{ old('status', $event->status) === 'Available' ? 'selected' : '' }}>Available
                            </option>
                            <option class="text-gray-900 dark:text-gray-100" value="Upcoming"
                                {{ old('status', $event->status) === 'Upcoming' ? 'selected' : '' }}>
                                Upcoming</option>
                            <option class="text-gray-900 dark:text-gray-100" value="Closed"
                                {{ old('status', $event->status) === 'Closed' ? 'selected' : '' }}>
                                Closed</option>
                        </select>
                    ...
                    <!-- Map Section -->
                    <div class="mt-5">
                        <label class="block text-primary dark:text-dark-primary font-semibold mb-2">Event
                            Location</label>
                        ...
                        <div class="mt-3 flex items-center justify-between">
                            <div id="selectedLocation" class="text-sm text-gray-600 dark:text-gray-300">
                                @if (old('latitude', $event->latitude) && old('longitude', $event->longitude))
                                    Current location: {{ old('latitude', $event->latitude) }},
                                    {{ old('longitude', $event->longitude) }}
                                @else
            ...
            function initEditMap() {
                ...
                marker.on('dragend', function(e) {
                    ...
                    geocoder.reverse({
                        lat,
                        lng
                    }, editMapInstance.options.crs.scale(editMapInstance.getZoom()), function(results) {
                        if (results.length > 0) {
                ...
                editMapInstance.on('click', function(e) {
                    ...
                    // Reverse geocode to get address
                    geocoder.reverse(e.latlng, editMapInstance.options.crs.scale(editMapInstance.getZoom()), function(
                        results) {
                        if (results.length > 0) {
                ...
                @if ($event->latitude && $event->longitude)
                    ...
                    googleMapsLink.href =
                        `https://www.google.com/maps?q=${initialLat},${initialLng}&ll=${initialLat},${initialLng}&z=15`;
                @else
